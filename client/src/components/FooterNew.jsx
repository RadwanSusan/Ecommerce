import React from "react";
import "./footerNew.css";
import {
	AiOutlineHome,
	AiOutlinePhone,
	AiOutlineMail,
	AiOutlineTwitter,
	AiOutlineInstagram,
	AiOutlineCreditCard,
	AiOutlineLike,
} from "react-icons/ai";
import { FaFacebookF, FaPinterest, FaLinkedinIn } from "react-icons/fa";
import { BiSupport } from "react-icons/bi";
import { GiTicket, GiWorld } from "react-icons/gi";

const FooterNew = () => {
	return (
		<div
			id="foodter_v1"
			class="footer footer-v1 snipcss-Ehoar tether-target-attached-top tether-element-attached-top tether-element-attached-center tether-target-attached-center"
		>
			<div class="footer-top footer-top-1">
				<div class="container">
					<aside
						id="automatic-static-block-2"
						class="widget automatic_widget_staticblock"
					>
						<div class="automatic-widget automatic-static-block">
							<div class="vc_row wpb_row vc_row-fluid">
								<div class="wpb_column vc_column_container vc_col-sm-12">
									<div class="vc_column-inner">
										<div class="wpb_wrapper">
											<div class="automatic-icon list au_fadeIn animated fadeIn">
												<div class="text-center icon-size-md">
													<div class="line-row">
														<div class="border col-xs-15 col-sm-15 col-md-15 col-lg-15 col-xl-15 first">
															<div class="icon">
																<span class="icon">
																	<GiTicket />
																</span>
															</div>
															<div class="box-content">
																<h4 class="title-icon">GREAT VALUE</h4>
																<div>
																	We offer competitive prices on our 100 million
																	plus product range.
																</div>
															</div>
														</div>
														<div class="border col-xs-15 col-sm-15 col-md-15 col-lg-15 col-xl-15 ">
															<div class="icon">
																<span class="icon">
																	<GiWorld />
																</span>
															</div>
															<div class="box-content">
																<h4 class="title-icon">WORLDWIDE DELIVERY</h4>
																<div>
																	With sites in 5 languages, we ship to over 200
																	countries &amp; regions.
																</div>
															</div>
														</div>
														<div class="border col-xs-15 col-sm-15 col-md-15 col-lg-15 col-xl-15 ">
															<div class="icon">
																<span class="icon">
																	<AiOutlineCreditCard />
																</span>
															</div>
															<div class="box-content">
																<h4 class="title-icon">SAFE PAMENT</h4>
																<div>
																	Pay with the world’s most popular and secure
																	payment methods.
																</div>
															</div>
														</div>
														<div class="border col-xs-15 col-sm-15 col-md-15 col-lg-15 col-xl-15 ">
															<div class="icon">
																<span class="icon">
																	<AiOutlineLike />
																</span>
															</div>
															<div class="box-content">
																<h4 class="title-icon">SHOP WITH CONFIDENCE</h4>
																<div>
																	Our Buyer Protection covers your purchase from
																	click to delivery.
																</div>
															</div>
														</div>
														<div class="border col-xs-15 col-sm-15 col-md-15 col-lg-15 col-xl-15 ">
															<div class="icon">
																<span class="icon">
																	<BiSupport />
																</span>
															</div>
															<div class="box-content">
																<h4 class="title-icon">24/7 HELP CENTER</h4>
																<div>
																	Round-the-clock assistance for a smooth
																	shopping experience.
																</div>
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
					</aside>
				</div>
			</div>
			<div class="container">
				<div class="footer-center padding-bottom-100">
					<aside
						id="automatic-static-block-3"
						class="widget automatic_widget_staticblock"
					>
						<div class="automatic-widget automatic-static-block">
							<div
								data-vc-full-width="true"
								data-vc-full-width-init="true"
								class="vc_row wpb_row vc_row-fluid home1-newletter background-default vc_custom_1489394616135 snipcss0-0-0-1 tether-target-attached-top tether-element-attached-top tether-element-attached-center tether-target-attached-center style-4TY4e"
								id="style-4TY4e"
							>
								<div class="wpb_column vc_column_container vc_col-sm-7 snipcss0-1-1-2">
									<div class="vc_column-inner snipcss0-2-2-3">
										<div class="wpb_wrapper snipcss0-3-3-4">
											<div class="wpb_widgetised_column wpb_content_element snipcss0-4-4-5">
												<div class="wpb_wrapper snipcss0-5-5-6">
													<aside
														id="mc4wp_form_widget-3"
														class="widget widget_mc4wp_form_widget snipcss0-6-6-7"
													>
														<form
															id="mc4wp-form-1"
															class="mc4wp-form mc4wp-form-200 snipcss0-7-7-8"
															method="post"
															data-id="200"
															data-name=""
														>
															<div class="mc4wp-form-fields snipcss0-8-8-9">
																<p class="snipcss0-9-9-10">
																	<label class="snipcss0-10-10-11">
																		Subscription
																	</label>
																	<input
																		type="email"
																		name="EMAIL"
																		placeholder="Your email address ..."
																		required=""
																		class="snipcss0-10-10-12"
																	/>
																	<span class="submit-over snipcss0-10-10-13">
																		<input
																			type="submit"
																			value="Subscribe us"
																			class="snipcss0-11-13-14"
																		/>
																	</span>
																</p>
															</div>
															<label
																class="snipcss0-8-8-15 style-U78xg"
																id="style-U78xg"
															>
																Leave this field empty if you're human:
																<input
																	type="text"
																	name="_mc4wp_honeypot"
																	value=""
																	tabindex="-1"
																	autocomplete="off"
																	class="snipcss0-9-15-16"
																/>
															</label>
															<input
																type="hidden"
																name="_mc4wp_timestamp"
																value="1679394145"
																class="snipcss0-8-8-17"
															/>
															<input
																type="hidden"
																name="_mc4wp_form_id"
																value="200"
																class="snipcss0-8-8-18"
															/>
															<input
																type="hidden"
																name="_mc4wp_form_element_id"
																value="mc4wp-form-1"
																class="snipcss0-8-8-19"
															/>
															<div class="mc4wp-response snipcss0-8-8-20"></div>
														</form>
													</aside>
												</div>
											</div>
										</div>
									</div>
								</div>
								<div class="wpb_column vc_column_container vc_col-sm-5 snipcss0-1-1-21">
									<div class="vc_column-inner vc_custom_1486623379385 snipcss0-2-21-22">
										<div class="wpb_wrapper snipcss0-3-22-23">
											<div class="wpb_text_column wpb_content_element snipcss0-4-23-24">
												<div class="wpb_wrapper snipcss0-5-24-25">
													<div class="call-us snipcss0-6-25-26">
														GOT QUESTIONS? CALL US 24/7
														<i class="automaticicon-phone snipcss0-7-26-27"></i>
														<span class="call-phone snipcss0-7-26-28">
															(40) 1257 7058
														</span>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
							<div class="vc_row-full-width vc_clearfix"></div>
						</div>
					</aside>
				</div>
				<div class="footer-logo">
					<aside
						id="automatic-logo-2"
						class="widget automatic_widget_logo snipcss0-1-1-2"
					>
						<div class="footer-logo">
							<a href="" class="snipcss0-3-3-4">
								<img
									src="https://emarche.net/wp-content/uploads/2017/02/Logo-resized.png"
									alt="eMarche"
									class="snipcss0-4-4-5"
								/>
							</a>
							<span class="text snipcss0-3-3-6">
								everything you love, in one place..
							</span>
						</div>
					</aside>
				</div>
				<div class="footer-top">
					<div class="row snipcss0-0-0-1 tether-target-attached-top tether-element-attached-top tether-element-attached-center tether-target-attached-center">
						<div class="col-md-8 col-lg-8 col-sm-7 col-xs-12 middle-right snipcss0-1-1-2">
							<div class="row snipcss0-2-2-3">
								<div class="col-md-6 col-lg-6 col-sm-12 col-xs-12 snipcss0-3-3-4 ipad">
									<div class="row snipcss0-4-4-5">
										<div class="col-md-6 col-lg-6 col-sm-6 col-xs-6 snipcss0-5-5-6">
											<aside
												id="text-2"
												class="widget widget_text snipcss0-6-6-7"
											>
												<h3 class="widget-title snipcss0-7-7-8">INFORMATION</h3>
												<div class="textwidget snipcss0-7-7-9">
													<ul id="menu-infomation" class="menu snipcss0-8-9-10">
														<li
															id="menu-item-2347"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2347 snipcss0-9-10-11"
														>
															<a
																href="https://emarche.net/#"
																class="snipcss0-10-11-12"
															>
																About store
															</a>
														</li>
														<li
															id="menu-item-2348"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2348 snipcss0-9-10-13"
														>
															<a
																href="#"
																class="snipcss0-10-13-14"
															>
																New collections
															</a>
														</li>
														<li
															id="menu-item-2349"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2349 snipcss0-9-10-15"
														>
															<a
																href="#"
																class="snipcss0-10-15-16"
															>
																Woman dress
															</a>
														</li>
														<li
															id="menu-item-2350"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2350 snipcss0-9-10-17"
														>
															<a
																href="#"
																class="snipcss0-10-17-18"
															>
																Contact us
															</a>
														</li>
														<li
															id="menu-item-2351"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2351 snipcss0-9-10-19"
														>
															<a
																href="#"
																class="snipcss0-10-19-20"
															>
																Latest news
															</a>
														</li>
														<li
															id="menu-item-2352"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2352 snipcss0-9-10-21"
														>
															<a
																href="#"
																class="snipcss0-10-21-22"
															>
																Our sitemap
															</a>
														</li>
													</ul>
												</div>
											</aside>
										</div>
										<div class="col-md-6 col-lg-6 col-sm-6 col-xs-6 snipcss0-5-5-23">
											<aside
												id="nav_menu-3"
												class="widget widget_nav_menu snipcss0-6-23-24"
											>
												<div class="menu-location-container snipcss0-7-24-25">
													<ul id="menu-location" class="menu snipcss0-8-25-26">
														<li
															id="menu-item-2375"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2375 snipcss0-9-26-27"
														>
															<a href="#" class="snipcss0-10-27-28">
																New York
															</a>
														</li>
														<li
															id="menu-item-2376"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2376 snipcss0-9-26-29"
														>
															<a href="#" class="snipcss0-10-29-30">
																London SF
															</a>
														</li>
														<li
															id="menu-item-2377"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2377 snipcss0-9-26-31"
														>
															<a href="#" class="snipcss0-10-31-32">
																Cockfosters BP
															</a>
														</li>
														<li
															id="menu-item-2378"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2378 snipcss0-9-26-33"
														>
															<a href="#" class="snipcss0-10-33-34">
																Los Angeles
															</a>
														</li>
														<li
															id="menu-item-2379"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2379 snipcss0-9-26-35"
														>
															<a href="#" class="snipcss0-10-35-36">
																Chicago
															</a>
														</li>
														<li
															id="menu-item-2380"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2380 snipcss0-9-26-37"
														>
															<a href="#" class="snipcss0-10-37-38">
																Las Vegas
															</a>
														</li>
													</ul>
												</div>
											</aside>
										</div>
									</div>
								</div>
								<div class="col-md-6 col-lg-6 col-sm-12 col-xs-12 snipcss0-3-3-39">
									<div class="row snipcss0-4-39-40">
										<div class="col-md-6 col-lg-6 col-sm-6 col-xs-6 snipcss0-5-40-41">
											<aside
												id="nav_menu-4"
												class="widget widget_nav_menu snipcss0-6-41-42"
											>
												<div class="menu-useful-links-container snipcss0-7-42-43">
													<ul
														id="menu-useful-links"
														class="menu snipcss0-8-43-44"
													>
														<li
															id="menu-item-2381"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2381 snipcss0-9-44-45"
														>
															<a href="#" class="snipcss0-10-45-46">
																Privacy Policy
															</a>
														</li>
														<li
															id="menu-item-2382"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2382 snipcss0-9-44-47"
														>
															<a href="#" class="snipcss0-10-47-48">
																Returns
															</a>
														</li>
														<li
															id="menu-item-2383"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2383 snipcss0-9-44-49"
														>
															<a href="#" class="snipcss0-10-49-50">
																Terms &amp; Conditions
															</a>
														</li>
														<li
															id="menu-item-2384"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2384 snipcss0-9-44-51"
														>
															<a href="#" class="snipcss0-10-51-52">
																Contact Us
															</a>
														</li>
														<li
															id="menu-item-2385"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2385 snipcss0-9-44-53"
														>
															<a href="#" class="snipcss0-10-53-54">
																Latest News
															</a>
														</li>
														<li
															id="menu-item-2386"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2386 snipcss0-9-44-55"
														>
															<a href="#" class="snipcss0-10-55-56">
																Our Sitemap
															</a>
														</li>
													</ul>
												</div>
											</aside>
										</div>
										<div class="col-md-6 col-lg-6 col-sm-6 col-xs-6 snipcss0-5-40-57">
											<aside
												id="nav_menu-5"
												class="widget widget_nav_menu snipcss0-6-57-58"
											>
												<div class="menu-menu-container snipcss0-7-58-59">
													<ul id="menu-menu" class="menu snipcss0-8-59-60">
														<li
															id="menu-item-2387"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2387 snipcss0-9-60-61"
														>
															<a href="#" class="snipcss0-10-61-62">
																Instagram
															</a>
														</li>
														<li
															id="menu-item-2388"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2388 snipcss0-9-60-63"
														>
															<a href="#" class="snipcss0-10-63-64">
																Facebook
															</a>
														</li>
														<li
															id="menu-item-2389"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2389 snipcss0-9-60-65"
														>
															<a href="#" class="snipcss0-10-65-66">
																Contact Us
															</a>
														</li>
														<li
															id="menu-item-2390"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2390 snipcss0-9-60-67"
														>
															<a href="#" class="snipcss0-10-67-68">
																Latest News
															</a>
														</li>
														<li
															id="menu-item-2391"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2391 snipcss0-9-60-69"
														>
															<a href="#" class="snipcss0-10-69-70">
																Purchase Theme
															</a>
														</li>
														<li
															id="menu-item-2392"
															class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2392 snipcss0-9-60-71"
														>
															<a href="#" class="snipcss0-10-71-72">
																F.A.Q
															</a>
														</li>
													</ul>
												</div>
											</aside>
										</div>
									</div>
								</div>
							</div>
						</div>
						<div class="col-md-4 col-lg-4 col-sm-5 col-xs-12 middle-left snipcss0-1-1-73">
							<aside
								id="automatic-contact-2"
								class="widget automatic_widget_contact snipcss0-2-73-74"
							>
								<h3 class="widget-title snipcss0-3-74-75">Connect Us</h3>
								<div class="footer-info-v1 snipcss0-3-74-76">
									<div class="links snipcss0-4-76-77">
										<ul class="snipcss0-5-77-78">
											<li class="snipcss0-6-78-79">
												<em class="automaticicon-home snipcss0-7-79-80">
													<AiOutlineHome />
												</em>
												<span class="text snipcss0-7-79-81">
													Pasig City, Philippines
												</span>
											</li>
											<li class="snipcss0-6-78-82">
												<em class="automaticicon-phone snipcss0-7-82-83">
													<AiOutlinePhone />
												</em>
												<a href="tel:+639668461690" class="snipcss0-7-82-84">
													Tel.
													<span class="text snipcss0-8-84-85">
														+639668461690
													</span>
												</a>
											</li>
											<li class="snipcss0-6-78-86">
												<em class="automaticicon-mail snipcss0-7-86-87">
													<AiOutlineMail />
												</em>
												<a
													href="mailto:support@emarche.net"
													class="snipcss0-7-86-88"
												>
													Mail.
													<span class="text snipcss0-8-88-89">
														support@emarche.net
													</span>
												</a>
											</li>
										</ul>
									</div>
								</div>
							</aside>
							<aside
								id="automatic-social-2"
								class="widget automatic_widget_social snipcss0-2-73-90"
							>
								<div class="social-login-options snipcss0-3-90-91">
									<div class="social snipcss0-4-91-92">
										<a href="#" target="_blank" class="snipcss0-5-92-93">
											<FaFacebookF />
										</a>
									</div>
									<div class="social snipcss0-4-91-95">
										<a href="#" target="_blank" class="snipcss0-5-95-96">
											<AiOutlineTwitter />
										</a>
									</div>
									<div class="social snipcss0-4-91-98">
										<a href="#" target="_blank" class="snipcss0-5-98-99">
											<FaPinterest />
										</a>
									</div>
									<div class="social snipcss0-4-91-101">
										<a href="#" target="_blank" class="snipcss0-5-101-102">
											<AiOutlineInstagram />
										</a>
									</div>
									<div class="social snipcss0-4-91-104">
										<a href="#" target="_blank" class="snipcss0-5-104-105">
											<FaLinkedinIn />
										</a>
									</div>
								</div>
							</aside>
						</div>
					</div>
				</div>
			</div>
			<div class="bottom-footer">
				<div class="container">
					<div class="row">
						<div class="col-md-6 col-lg-6 col-sm-6 col-xs-12">
							<div class="pull-left footercopyright">
								© 2022 eMARCHE. ALL RIGHTS RESERVED | POWERED BY
								<a href="#">PME</a>
							</div>
						</div>
						<div class="col-md-6 col-lg-6 col-sm-6 col-xs-12">
							<div class="pull-right">
								<img
									class="alignnone wp-image-368 size-medium"
									src="https://emarche.net/wp-content/uploads/2017/01/bank-2-300x26.png"
									alt=""
									width="300"
									height="26"
								/>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	);
};
export default FooterNew;
